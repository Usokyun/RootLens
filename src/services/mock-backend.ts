...
function readPersistentState(): MockPersistentState {
  ...
  try {
    ...
    return {
      uploaded_runs: Array.isArray(parsed.uploaded_runs) ? (parsed.uploaded_runs as RunDetail[]) : [],
...
function buildTopKPaths(caseItem: RootLensRuntimeCase): Array<Record<string, unknown>> {
...
function buildRootCauseFromPath(path: RankedPath, rank: number, caseItem: RootLensRuntimeCase): RankedRootCause {
  return {
    ...
    candidate_role: caseItem.dataset === 'mvtec' ? 'plausible_mechanism' : 'candidate_root_cause',
    score: path.score,
    confidence: path.confidence,
    evidence_match: path.evidence_match,
...
function buildRootCauseFromRoute2(candidate: RootKGDCandidate): RankedRootCause {
  return {
...
function buildRankedRootCauses(caseItem: RootLensRuntimeCase): RankedRootCause[] {
  const route2Candidates = caseItem.analysis.route2?.ranked_candidates ?? []
  if (route2Candidates.length) {
    return route2Candidates.slice(0, 6).map((candidate) => buildRootCauseFromRoute2(candidate))
  }

  const route1Paths = caseItem.analysis.route1?.ranked_paths ?? []
  return route1Paths.slice(0, 6).map((path, index) => buildRootCauseFromPath(path, index + 1, caseItem))
}

function collectSourceEdgeProvenance(topKPaths: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  const edges: Array<Record<string, unknown>> = []

  for (const path of topKPaths) {
    const sourceEdges = Array.isArray(path.source_edges) ? path.source_edges : []
...
function pathGraphFromPaths(topKPaths: Array<Record<string, unknown>>): PathGraph {
  ...
  topKPaths.forEach((path, index) => {
    ...
    const nodes = Array.isArray(path.nodes) ? path.nodes.map((item) => String(item)) : []
    const nodeNames = Array.isArray(path.node_names) ? path.node_names.map((item) => String(item)) : []
    ...
    const graphNodes = nodes.map((nodeId, nodeIndex) => {
      ...
      return {
        ...
        role: nodeIndex === 0 ? 'source' : nodeIndex === nodes.length - 1 ? 'target' : 'intermediate',
      }
    })
    ...
    const graphEdges = relations.map((relation, edgeIndex) => {
      ...
      return {
        ...
        relation,
        source: typeof sourceRecord.source === 'string' ? sourceRecord.source : null,
        evidence: typeof sourceRecord.evidence === 'string' ? sourceRecord.evidence : null,
        confidence: typeof sourceRecord.confidence === 'number' ? sourceRecord.confidence : null,
        review_status: typeof sourceRecord.review_status === 'string' ? sourceRecord.review_status : 'auto',
      }
    })
    ...
    paths.push({
      ...
      source_entity_id: typeof path.source_entity_id === 'string' ? path.source_entity_id : null,
      target_entity_id: typeof path.target_entity_id === 'string' ? path.target_entity_id : null,
      score: typeof path.score === 'number' ? path.score : null,
      confidence: typeof path.confidence === 'number' ? path.confidence : null,
      supporting_evidence: Array.isArray(path.supporting_evidence) ? path.supporting_evidence : [],
      nodes: graphNodes,
      edges: graphEdges,
    })
  })
...
function reviewTargetsFromArtifacts(input: {
  ...
  input.rankedRootCauses.forEach((rootCause) => {
    const rankingId = normalizeText(rootCause.ranking_id || rootCause.candidate_id)
    if (!rankingId) {
      return
    }
  ...
  input.linkedEntities.forEach((link) => {
    ...
    targets.push({
      ...
      label: normalizeText(link.selected_entity_name ?? link.selected_entity_id ?? linkId, linkId),
    })
  })
...
function buildVisualEvidence(caseItem: RootLensRuntimeCase): VisualEvidenceItem[] {
  if (caseItem.dataset === 'tep') {
    return []
  }
...
function buildCaseDetail(caseItem: RootLensRuntimeCase): RunCaseDetail {
  const linkedEntities = deepClone(
    (caseItem.analysis.route1?.linked_entities ?? []) as unknown as Array<Record<string, unknown>>,
  )
  const correctionCandidates = deepClone(
    (caseItem.analysis.route1?.correction_candidates ?? []) as unknown as Array<Record<string, unknown>>,
  )
  ...
  return {
    ...
    generated_evidence: deepClone(caseItem.evidence as unknown as Record<string, unknown>),
...
function aggregateCases(caseDetails: RunCaseDetail[]) {
  ...
  const topKPaths = caseDetails.flatMap((item) => item.top_k_paths ?? []).slice(0, 24)
  const rankedRootCauses = caseDetails.flatMap((item) => item.ranked_root_causes ?? []).slice(0, 24)
  const linkedEntities = caseDetails.flatMap((item) => item.linked_entities ?? []).slice(0, 48)
  const correctionCandidates = caseDetails.flatMap((item) => item.correction_candidates ?? []).slice(0, 48)
  ...
  const visualEvidence = caseDetails.flatMap((item) => item.visual_evidence ?? [])
  ...
  return {
    ...
    evidence_summary: aggregate.firstCase
      ? {
          ...
          observation_count: Array.isArray(aggregate.firstCase.generated_evidence?.observations)
...
function buildKgStudioReviewTargets(edges: KGStudioGraphEdge[]): ReviewTarget[] {
...
async function buildSeedKgStudio(): Promise<{ kgStudio: KGStudioPayload; build: MockConstructionBuild }> {
  const [nodesCsv, edgesCsv, validationReport, generationSummary] = await Promise.all([
    ...
    fetchPublicJson<Record<string, unknown>>('/generated/mvtec-candidate-kg/validation_report.json'),
    fetchPublicJson<Record<string, unknown>>('/generated/mvtec-candidate-kg/kg_generation_summary.json'),
  ])
  ...
  const reviewStatusCounts = countBy(graphEdges.map((edge) => edge.review_status))
  ...
  const buildRecord: KGConstructionBuildRecord = {
    ...
    source_ids: ['wm811k_low_confidence_investigation_rule', 'wafer_factory_sop_private_summary'],
    source_count: 2,
  ...
  const reviewQueueEdges: KGConstructionReviewQueueEdge[] = edgeRows.map((row) => ({
  ...
  return {
    kgStudio: {
      ...
      validation_summary: (validationReport.summary as Record<string, unknown>) ?? validationReport,
      ...
      review_targets: reviewTargets.map((target) => ({
        ...
        label: target.label,
        source: graphEdges.find((edge) => edge.edge_id === target.target_id)?.source ?? '',
        confidence: graphEdges.find((edge) => edge.edge_id === target.target_id)?.confidence ?? null,
        review_status: graphEdges.find((edge) => edge.edge_id === target.target_id)?.review_status ?? 'auto',
      })),
...
function selectSeedRun(seedRuns: RunDetail[], options: { dataset?: string; mode?: UploadMode }): RunDetail {
  return (
    seedRuns.find((run) => run.run.dataset === options.dataset && run.run.mode === options.mode) ??
...
function buildEnvelopeFromCase(caseDetail: RunCaseDetail, note?: string): AnalyzeEnvelope {
  const evidence = deepClone((caseDetail.generated_evidence ?? {}) as Record<string, unknown>)
  const analysis = {
  ...
  return {
    case: {
      ...
      observation_count: Array.isArray(evidence.observations) ? evidence.observations.length : 0,
    },
    evidence,
    analysis,
...
function applyMockWhatIf(caseDetail: RunCaseDetail, request: WhatIfRequest): AnalyzeEnvelope {
  const envelope = buildEnvelopeFromCase(caseDetail, '模拟假设分析已重新计算并返回可复核的结果载荷。')
  envelope.evidence = {
    ...envelope.evidence,
    anomaly_type: request.anomaly_type,
    location: request.location ?? (envelope.evidence.location as string | null) ?? null,
    morphology: request.morphology ?? (envelope.evidence.morphology as string | null) ?? null,
    severity: request.severity ?? (envelope.evidence.severity as number | null) ?? null,
    confidence: request.confidence ?? (envelope.evidence.confidence as number | null) ?? null,
    raw_evidence: {
      ...(isRecord(envelope.evidence.raw_evidence) ? envelope.evidence.raw_evidence : {}),
  ...
  if (rankedRootCauses.length > 1) {
    ...
    envelope.analysis.ranked_root_causes = [
      ...
      { ...first, rank: 2, score: Math.max(0, normalizeScore(first.score, 0) - 0.02) },
...
function parseDraftLines(request: KGSourceDraftRequest): KGSourceDraftEdge[] {
  ...
  return lines.map((line, index) => {
    ...
    const [head, relation, tail, scenario = request.default_scenario, evidence = line] = parts
...
function parseConstructionSourceRows(source: KGConstructionBuildRequest['sources'][number]): Array<Record<string, unknown>> {
  ...
  if (rows.length) {
    return compactList(
      rows.map((row, index) => {
        ...
        const relation = normalizeText(row.relation ?? row.predicate ?? row.edge_type)
        ...
        const sourceId = normalizeText(row.source ?? row.source_id, source.source_id)
        return {
...
function buildConstructionReviewSummary(edges: KGConstructionReviewQueueEdge[]): KGConstructionReviewQueueSummary {
  return {
...
function buildConstructionBuildFromRequest(request: KGConstructionBuildRequest): MockConstructionBuild {
  const createdAt = new Date().toISOString()
  const runId = request.run_id || `mock-build-${Date.now()}`
  const sourceIds = request.sources.map((source) => source.source_id)
  const edges: KGConstructionReviewQueueEdge[] = request.sources.flatMap((source, sourceIndex) => {
    ...
    return [
      {
        ...
        evidence: source.path ?? source.source_text ?? 'Mock construction source',
        confidence: 0.58,
        weight: 0.51,
        review_status: 'auto',
        feedback_count: 0,
...
function filterConstructionEdges(
  ...
  return edges.filter((edge) => {
    ...
    if (request.query) {
      const haystack = [edge.head, edge.relation, edge.tail, edge.source, edge.evidence].join(' ').toLowerCase()
...
function persistConstructionBuild(build: MockConstructionBuild) {
  const state = readPersistentState()
  state.construction_builds = uniqueByKey([build, ...state.construction_builds], (item) => item.build.run_id)
  writePersistentState(state)
}

function mergedConstructionBuilds(seed: SeedBundle, persistentState: MockPersistentState): MockConstructionBuild[] {
  return uniqueByKey([...persistentState.construction_builds, ...seed.constructionBuilds], (item) => item.build.run_id)
}
...
export const mockBackend = {
  ...
  async analyze(request: AnalyzeRequest): Promise<AnalyzeEnvelope> {
    ...
    const seedRun = selectSeedRun(runs, { dataset: request.evidence?.dataset ?? undefined })
  ...
  async submitReview(request: ReviewRequest): Promise<{ status: string; record: Record<string, unknown> }> {
  ...
  async generateKGSourceDraft(request: KGSourceDraftRequest): Promise<KGSourceDraftResponse> {
    return {
      provider: 'heuristic',
  ...
  async submitKGDraft(request: KGDraftRequest): Promise<{ status: string; record: Record<string, unknown> }> {
  ...
  async buildKGConstruction(request: KGConstructionBuildRequest): Promise<KGConstructionBuildResponse> {
  ...
  async getKGConstructionBuild(runId: string): Promise<KGConstructionBuildDetail> {
    const { seed, persistentState } = await loadCurrentState()
    const build = mergedConstructionBuilds(seed, persistentState).find((item) => item.build.run_id === runId)
    if (!build) {
  ...
  async validateKGConstructionBuild(runId: string): Promise<KGConstructionBuildValidationResponse> {
    const { seed, persistentState } = await loadCurrentState()
    const build = mergedConstructionBuilds(seed, persistentState).find((item) => item.build.run_id === runId)
    if (!build) {
      throw new Error(`未找到构建记录：${runId}`)
  ...
  async reviewKGConstructionEdge(
    ...
    const reviewedBuild: MockConstructionBuild = {
      ...
      edges: existing.edges.map((edge) =>
        edge.target_key === targetKey
          ? {
              ...edge,
              review_status: request.action === 'accept' ? 'reviewed' : 'rejected',
              feedback_count: edge.feedback_count + 1,
              accepted_count: edge.accepted_count + (request.action === 'accept' ? 1 : 0),
              rejected_count: edge.rejected_count + (request.action === 'reject' ? 1 : 0),
            }
          : edge,
      ),
      qa_report: {
    ...
    const edge = reviewedBuild.edges.find((item) => item.target_key === targetKey)
    return {
  ...
  async uploadKGConstructionSource(input: {
    ...
    const uploadedSource: KGConstructionUploadedSource = {
      ...
      source_type: (input.source_type as KGConstructionUploadedSource['source_type']) ?? 'manual_table',
      scenario: input.scenario ?? 'shared',
      source_format: (input.source_format as KGConstructionUploadedSource['source_format']) ?? 'csv',
