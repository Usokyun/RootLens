...
- `/graphs` 需要支持总图 → 局部子图 → 根因列表联动：点击另一个候选节点时切换右侧候选，点击非候选节点时清空右侧候选高亮，点击空白或再次点击当前节点时退出总图选择。
- `/graphs` 右侧应收敛为整合后的“紧凑上下文 + 根因列表”常驻卡，不再出现独立“图谱提示”卡。
- `/graphs` 的“审阅与策展卡” segmented toggle 与动作按钮尺寸应稳定，hover / active 不应造成视觉尺寸跳变。
...
4. 刷新页面后的会话恢复。
5. 切回 demo 后的状态重置。
6. `/graphs` 中验证：右侧候选 → 总图候选节点 → 空白取消 → 重复点击取消 → 非候选节点清空根因高亮。
7. `/graphs` 中验证：右侧整合卡下边界与“审阅与策展卡”底部对齐，且 segmented toggle / 动作按钮无尺寸跳变。
