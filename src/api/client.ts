...
const BACKEND_PROXY_TARGET = import.meta.env.VITE_BACKEND_PROXY_TARGET ?? DEFAULT_API_BASE_URL
...
function buildUrl(baseUrl: string, path: string, search?: URLSearchParams): string {
...
async function requestJson<T>(baseUrl: string, path: string, init?: RequestInit, search?: URLSearchParams): Promise<T> {
...
export function createApiClient(baseUrl = DEFAULT_API_BASE_URL): ApiClient {
  return {
    bootstrap: () => requestJson<DashboardBootstrap>(baseUrl, '/api/dashboard/bootstrap'),
    ...
    analyze: (request) => requestJson<AnalyzeEnvelope>(baseUrl, '/api/analyze', jsonInit('POST', request)),
    whatIf: (request) => requestJson<AnalyzeEnvelope>(baseUrl, '/api/what-if', jsonInit('POST', request)),
    ...
    generateKGSourceDraft: (request) =>
      requestJson<KGSourceDraftResponse>(baseUrl, '/api/kg/source-draft', jsonInit('POST', request)),
    ...
    listKGConstructionBuilds: () =>
      requestJson<KGConstructionBuildListResponse>(baseUrl, '/api/kg/construction/builds'),
    getKGConstructionBuild: (runId) =>
      requestJson<KGConstructionBuildDetail>(baseUrl, `/api/kg/construction/builds/${runId}`),
    ...
    getKGConstructionReviewQueue: (runId, request) => {
      ...
      if (request?.review_status) search.set('review_status', request.review_status)
    ...
    listKGConstructionSources: () =>
      requestJson<KGConstructionSourceListResponse>(baseUrl, '/api/kg/construction/sources'),
    uploadKGConstructionSource: (input) => {
      const form = new FormData()
      ...
      if (input.source_format) form.append('source_format', input.source_format)
    ...
    artifactUrl: (runId, artifactName) => buildUrl(baseUrl, `/api/runs/${runId}/artifacts/${artifactName}`),
  }
}
